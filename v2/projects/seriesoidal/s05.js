...
// <div id="sketch-container05"></div>

function interfaz(){
  waves = 30; //Cuantas ondas
  frameLimit = 30; //cuándo se detiene? [en SEGUNDOS]
  margin = 0; //Dónde rebotan?
  rebotA = 0; // 1 = SI, 0 = NO

  for (let i = 0; i < waves; i++) {
      p[i] = new Particle(
        (width-i*15)-550, //random(margin, width - margin),
        i*30-100, //random(margin, height - margin), //x,y
        0.5,
        1, //velocidadX,velocidadY
        0,
        1*i/5, //alterX, alterY
        2, //diam
...
function lisay(y, dy, ay) {
  return y + dy * sin(radians((count-pCount) * ay))
           + dy * sin(radians(3*(count-pCount) * ay));
           + dy * sin(radians(5*(count-pCount) * ay));
           //+ dy * sin(radians(4*frameCount * ay));
}
function tinte(x) {
  // ¿Cómo cambia el tinte?
  return 180 * sin(radians((count-pCount)) + x);
}
...
function resetSketch(){
  ...
  canvas.parent('sketch-container05');
