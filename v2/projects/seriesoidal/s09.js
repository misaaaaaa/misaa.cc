...
// <div id="sketch-container09"></div>

function interfaz(){
  waves = 50; //Cuantas ondas
  frameLimit = 60; //cuándo se detiene? [en SEGUNDOS]
  margin = 0; //Dónde rebotan?
  rebotA = 1; // 1 = SI, 0 = NO

  for (let i = 0; i < waves; i++) {
      p[i] = new Particle(
        width/2,//random(margin, width - margin),
        i*15-15,//random(margin, height - margin), //x,y
        2+i/1000,
...
function lisay(y, dy, ay) {
  return y + dy * cos(radians((count-pCount) * ay)) * sin(radians((count-pCount) * ay))
}
function tinte(x) {
...
function resetSketch(){
  ...
  canvas.parent('sketch-container09');
