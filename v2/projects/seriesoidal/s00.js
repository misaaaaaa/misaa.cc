...
// <div id="sketch-container00"></div>

function interfaz(){
  waves = 50; //Cuantas ondas
  frameLimit = 10; //cuándo se detiene? [en SEGUNDOS]
  margin = 50; //Dónde rebotan?
  rebotA = 1; //Pregunta, 1 = SI, 0 = NO
  ...
  for (let i = 0; i < waves; i++) {
    p[i] = new Particle(
      ...
      random(-1,1),
      random(-1,1),      //velocidadX,velocidadY
      random(-1,1),
      random(-1,1), //Alteración freq X, freq Y
      random(-1,2), //Tamaño linea
...
function tinte(x){                                // ¿Cómo cambia el tinte?
  return 180 *cos(radians(count) + x);
}
...
function resetSketch(){
  ...
  canvas.parent('sketch-container00');
