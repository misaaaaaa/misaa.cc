...
// <div id="sketch-container08"></div>

function interfaz(){
  waves = 10; //Cuantas ondas
  frameLimit = 25; //cuándo se detiene? [en SEGUNDOS]
  margin = 40; //Dónde rebotan?
  rebotA = 0; // 1 = SI, 0 = NO

  for (let i = 1; i < waves; i++) {
      p[i] = new Particle(
        0,//random(margin, width - margin),
        height/2,//random(margin, height - margin), //x,y
        0.4155+i/10000,
        (i-waves/2)*-0.4, //velocidadX,velocidadY
        i/10,
        i/4, //alterX, alterY
        2, //diam
...
function lisax(x, dx, ax) {
  ...
  return x + dx //* cos(radians(frameCount * ax))
}
function lisay(y, dy, ay) {
  return y + dy * cos(radians((count-pCount) * ay)) * cos(radians((count-pCount) * ay)) * cos(radians((count-pCount) * ay)) //* cos(radians((count-pCount) * ay)) * cos(radians((count-pCount) * ay))
}
function tinte(x) {
  // ¿Cómo cambia el tinte?
  return x + 180 * cos(radians((count-pCount))) * cos(radians((count-pCount))) * cos(radians((count-pCount)));
}
...
function resetSketch(){
  ...
  canvas.parent('sketch-container08');
