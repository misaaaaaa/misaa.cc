...
// <div id="sketch-container03"></div>

function interfaz(){
  waves = 100; //Cuantas ondas
  frameLimit = 2; //cuándo se detiene? [en SEGUNDOS]
  ...
  for (let i = 0; i < waves; i++) {
    p[i] = new Particle(
      ...
      i*10,//random(margin, height - margin), //x,y
      1,
...
function lisax(x,dx,ax){                          //  ¿Cómo se mueven en el eje X?
  return x + dx * tan(radians((count-pCount) * ax));
}
function lisay(y,dy,ay){
  return y + dy * tan(radians((count-pCount) * ay));
}
function tinte(x){                                // ¿Cómo cambia el tinte?
  return 180 *cos(radians((count-pCount)) + x);
}
...
function resetSketch(){
  ...
  canvas.parent('sketch-container03');
