...
// <div id="sketch-container04"></div>

function interfaz(){

  waves = 15; //Cuantas ondas
  frameLimit = 10; //cuándo se detiene? [en SEGUNDOS]
  margin = 10; //Dónde rebotan?
  rebotA = 1; // 1 = SI, 0 = NO
  ...
  for (let i = 0; i < waves; i++) {
    p[i] = new Particle(
      ...
      height/2, //x,y
      i/2,
      i/2, //velocidadX,velocidadY
      i,
      i, //alterX, alterY
      1, //diam
      margin*i*3, // punto de rebote
...
function lisax(x,dx,ax){                          //  ¿Cómo se mueven en el eje X?
  return x   + dx * cos( PI/10  *radians((count-pCount) * ax))
}

function lisay(y,dy,ay){
  return y + dy * sin( PI/10 *radians((count-pCount) * ay))

}
function tinte(x){                                // ¿Cómo cambia el tinte?
  return x //+ 180 *cos(radians((count-pCount)) + x);
}
...
function resetSketch(){
  ...
  canvas.parent('sketch-container04');
