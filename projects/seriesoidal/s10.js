...
// <div id="sketch-container10"></div>

function interfaz(){

  waves = 50; //Cuantas ondas
  frameLimit = 362; //cuándo se detiene? [en SEGUNDOS]
  ...
  rebotA = 0; // 1 = SI, 0 = NO
      ...
      p[i] = new Particle(
        ...
        height-(i*15-15),//random(margin, height - margin), //x,y
        i/22+0.1,
        i/20, //velocidadX,velocidadY
        i/60,
        i/2-i, //alterX, alterY
        1.5, //diam
...
function lisay(y, dy, ay) {
  return y + dy * cos(radians((count-pCount) * ay))
}
function tinte(x) {
  // ¿Cómo cambia el tinte?
  return x + 180 * (sin(radians((count-pCount))))// * cos(radians((count-pCount))) * cos(radians((count-pCount)));
}
