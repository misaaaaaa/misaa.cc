...
// <div id="sketch-container07"></div>

function interfaz(){
  waves = 10; //Cuantas ondas
  frameLimit = 17.3; //cuándo se detiene? [en SEGUNDOS]
  margin = 40; //Dónde rebotan?
  rebotA = 1; // 1 = SI, 0 = NO

  for (let i = 0; i < waves; i++) {
      p[i] = new Particle(
        margin,//random(margin, width - margin),
        random(margin, height - margin), //x,y
        0.5,
        -(i+3)/10, //velocidadX,velocidadY
        i/10,
        random(i+1)/10, //alterX, alterY
        1, //diam
...
function resetSketch(){
  ...
  canvas.parent('sketch-container07');
